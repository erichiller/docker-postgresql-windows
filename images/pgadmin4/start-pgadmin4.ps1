...
$env:PGADMIN_WSGI = Join-Path $env:PGADMIN_DIR 'pgAdmin4.wsgi'
# 'C:\pgsql\pgAdmin 4\web\pgAdmin4.wsgi'

# $env:PGADMIN_ACTIVATE = 'C:/pgsql/pgAdmin 4/venv/Scripts/activate_this.py'
# $env:PGADMIN_ACTIVATE = Join-Path $env:PGADMIN_DIR 'setup.py'
...
if ( -not (  (Get-Content $env:APACHE_CONFIG ) -like '*mod_wsgi*')  ) {
    ...
    & python -m pip install mod-wsgi pgadmin4
...
if ( -not (  (Get-Content $env:APACHE_CONFIG ) -like "*Directory $($env:PGADMIN_DIR)*")  ) {
...
# C:\Python\lib\site-packages\pgadmin4\ 
...
CONSOLE_LOG_LEVEL = logging.INFO # INFO or DEBUG ?
...
if ( $( . python "$env:PGADMIN_SETUP" --dump-servers C:\server-dump.json) ) {
    ...
    # $env:PYTHONPATH = ( '{0};{1}' -f ( 'C:\\pgsql\\pgAdmin 4\\venv\\Lib\\site-packages', ( python -c 'import sys; print(chr(59).join(x for x in sys.path if x))') ) ) ; 
...
if ( $env:PGADMIN_ENABLE_TLS -eq "True" ) {
    ...
    Get-Content -Wait -Tail 100 -Path "C:/data/apache/logs/error.log"
}
